...
import St from 'gi://St';import Gio from 'gi://Gio';
...
export const AlbumArt = GObject.registerClass(
    class AlbumArt extends St.BoxLayout {
            ...
            this._settingsChangedId = this._settings.connect(
                ...
                () => {
                    if (!this._isDestroyed)
                        this._onVinylAppsSettingChanged();
                },
            );
        }
        ...
        _toggleVinylForCurrentPlayer() {
            ...
            // Fallback: no player info → toggle the legacy global key
            if (ids.size === 0) {
                const cur = this._settings.get_boolean(
            ...
            if (enabled) {
                ...
                const filtered = list.filter(
                    id => !isVinylEnabledForIds(ids, [id]),
                );
        ...
        _saveInstance(preferredId) {
            ...
            if (this._manager) {
                ...
                const de =
                    this._manager._desktopEntries?.get(this._playerName);
                if (de)
                    desktopId = de.endsWith('.desktop') ? de.slice(0, -8) : de;
            }
            ...
            try {
                const existing =
                    this._settings.get_strv('vinyl-app-instances') ?? [];
        ...
        _onVinylAppsSettingChanged() {
            ...
            // If vinyl just enabled but paused, tonearm is already parked
            // (stopRotation called moveToParked via the Tonearm widget)
        }
        ...
        _buildUI() {
            ...
            // Tonearm — sits in its own container above the disc
            ...
            // Tonearm widget (from tonearm.js)
        ...
        startRotation(isPlaying = true) {
            ...
            this._rotationInterval = GLib.timeout_add(
                ...
                interval,
                () => {
                    if (this._isDestroyed || !this._isRotating)
                        return GLib.SOURCE_REMOVE;

                    this._rotationAngle =
                        (this._rotationAngle + degPerInterval) % 360;
                    if (this._rotatingContainer)
                        this._rotatingContainer.rotation_angle_z =
                            this._rotationAngle;
        ...
        /** Internal: remove the rotation GLib source. */
        ...
        destroy() {
            ...
            // All GLib sources (review-guideline requirement)
            this.stopRotation();
