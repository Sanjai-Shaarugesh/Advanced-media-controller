...
export class IndicatorEventHandlers {
  ...
  connectControlSignals() {
    try {
      this._indicator._controls.connect("play-pause", () =>
        this._indicator._state.safeExecute(() => this._onPlayPause()),
      );
      this._indicator._controls.connect("next", () =>
        this._indicator._state.safeExecute(() => this._onNext()),
      );
      this._indicator._controls.connect("previous", () =>
        this._indicator._state.safeExecute(() => this._onPrevious()),
      );
      this._indicator._controls.connect("shuffle", () =>
        this._indicator._state.safeExecute(() => this._onShuffle()),
      );
      this._indicator._controls.connect("repeat", () =>
        this._indicator._state.safeExecute(() => this._onRepeat()),
      );
      this._indicator._controls.connect("seek", (_, position) =>
  ...
  setupSessionMonitoring() {
    this._indicator._state._sessionModeId = Main.sessionMode.connect("updated", () => {
      ...
      this._indicator._state.scheduleOperation(() => {
        if (this._indicator._state._isDestroyed || !this._indicator._state._managerInitialized ||
  ...
  setupWindowMonitoring() {
    ...
    try {
      this._indicator._state._capturedEventId = global.stage.connect("button-press-event", (actor, event) => {
        if (this._indicator._state._isDestroyed || !this._indicator.menu.isOpen ||
  ...
  _onPlayPause() {
    if (this._indicator._state._isDestroyed || !this._indicator._state._currentPlayer ||
        this._indicator._state._sessionChanging)
      return;
